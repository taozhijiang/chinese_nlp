                ...
                for line in fin:
                    ...
                    for item in line_t:
                        if item not in stop_words and hanzi_util.is_zhs(item):
                            ...
                            # train_data_single
                            if item_id not in train_data_single.keys():
                                train_data_single[item_id] = {}
                            if tag_id not in train_data_single[item_id].keys():
                    ...
                    for index_i in range(len(objs) - 1):
                        for index_j in range(index_i + 1, len(objs)):
                            ...
                            if objs[index_i] < objs[index_j]:
                                ...
                                item_i = objs[index_j]
                                item_j = objs[index_i]
                            if item_i in train_data.keys():
                                if item_j in train_data[item_i].keys():
                                    if tag_id in train_data[item_i][item_j].keys():
                                        train_data[item_i][item_j][tag_id] += 1
                                    else:
                                        train_data[item_i][item_j][tag_id] = 1
                                else:
                                    train_data[item_i][item_j] = {}
                                    train_data[item_i][item_j][tag_id] = 1
                            else:
                                train_data[item_i] = {}
                                train_data[item_i][item_j] = {}
...
def calc_vector(str):
    ...
    for item_tag in train_tags:
        ...
        for item_w in sub_train:
            ...
            count_s = sum(train_data_single[item_i].values()) + sum(train_data_single[item_2].values())
            count = 0
            if item_1 in train_data.keys() and \
            item_2 in train_data[item_1].keys() and \
            tag_id in train_data[item_1][item_2].keys():
                count +=  train_data[item_1][item_2][tag_id]
            ...
            for (item_1, item_1val) in train_data.items():
                ...
                if item_1val:
                    for (item_2, item_2val) in item_1val.items():
                        word_2 = train_word_id[item_2]
                        fout.write('\t'+word_2+':\n')
                        if item_2val:
                            for (tag_id, tag_val) in item_2val.items():
                                fout.write('\t\t'+train_tags[tag_id]+':'+str(tag_val)+'\n')
